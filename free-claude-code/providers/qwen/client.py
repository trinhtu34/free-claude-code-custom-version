...
QWEN_DEFAULT_BASE_URL = "http://localhost:3000/v1"


class QwenProvider(OpenAIChatTransport):
    ...
    def __init__(self, config: ProviderConfig):
        super().__init__(
            config,
            provider_name="QWEN",
            base_url=config.base_url or QWEN_DEFAULT_BASE_URL,
    ...
    def _get_retry_request_body(self, error: Exception, body: dict) -> dict | None:
        """Retry once with a downgraded body when Qwen rejects a known field."""
        ...
        if "reasoning_budget" in error_text:
            ...
            if retry_body is None:
                return None
            logger.warning(
                "QWEN_STREAM: retrying without reasoning_budget after 400 error"
            )
            return retry_body
        ...
        if "chat_template" in error_text:
            ...
            if retry_body is None:
                return None
            logger.warning("QWEN_STREAM: retrying without chat_template after 400 error")
            return retry_body
